...
use caliptra_emu_bus::{
    testing::{FakeBus, Log}, BusAccessType,
    Bus, BusError, Ram,
};
...
fn test_read_dispatch() {
    ...
    assert_eq!(bus.read(RvSize::Word, 0x3430, BusAccessType::DataLoad).unwrap(), 0x3828_abcd);

    bus.sram.write(RvSize::Word, 0x0, 0x892a_b38a).unwrap();
    assert_eq!(bus.read(RvSize::HalfWord, 0x1000_0000, BusAccessType::DataLoad).unwrap(), 0xb38a);

    bus.dram.write(RvSize::Word, 0xfffc, 0x2a29_3072).unwrap();
    assert_eq!(bus.read(RvSize::Byte, 0x2000_ffff, BusAccessType::DataLoad).unwrap(), 0x2a);

    bus.uart0.write(RvSize::Word, 0x20, 0x8e27_ab42).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa00_0020, BusAccessType::DataLoad).unwrap(), 0x8e27_ab42);

    bus.uart1.write(RvSize::Word, 0x74, 0x38b8_e201).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa01_0074, BusAccessType::DataLoad).unwrap(), 0x38b8_e201);

    bus.i2c0.write(RvSize::Word, 0x1c, 0xc1a4_823f).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa02_001c, BusAccessType::DataLoad).unwrap(), 0xc1a4_823f);

    bus.i2c0.write(RvSize::Word, 0x68, 0x0e8a_440b).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa02_0068, BusAccessType::DataLoad).unwrap(), 0x0e8a_440b);

    bus.i2c1.write(RvSize::Word, 0x00, 0x0e8a_440b).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa02_0400, BusAccessType::DataLoad).unwrap(), 0x0e8a_440b);

    bus.i2c2.write(RvSize::Word, 0x54, 0x70fa_81c9).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xaa02_0854, BusAccessType::DataLoad).unwrap(), 0x70fa_81c9);

    bus.spi0.write(RvSize::Word, 0xd87c, 0x48ba_38c1).unwrap();
    assert_eq!(bus.read(RvSize::Word, 0xbb42_d87c, BusAccessType::DataLoad).unwrap(), 0x48ba_38c1);

    assert_eq!(
        bus.read(RvSize::Word, 0x0001_0000, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Word, 0xf000_0000, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Word, 0xaa03_0000, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Word, 0xaa02_0900, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Word, 0xbb41_0000, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0d0, BusAccessType::DataLoad).unwrap(), 0xd149_b444);
    assert_eq!(bus.read(RvSize::HalfWord, 0xcafe_f0d4, BusAccessType::DataLoad).unwrap(), 0x695c);
    assert_eq!(bus.read(RvSize::Byte, 0xcafe_f0d8, BusAccessType::DataLoad).unwrap(), 0xc1);

    assert_eq!(
        bus.read(RvSize::HalfWord, 0xcafe_f0d0, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Word, 0xcafe_f0d4, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );

    assert_eq!(
        bus.read(RvSize::HalfWord, 0xcafe_f0f0, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0f4, BusAccessType::DataLoad).unwrap(), 0x1043_0000);
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0f8, BusAccessType::DataLoad).unwrap(), 0x1043_1000);
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0fc, BusAccessType::DataLoad).unwrap(), 0x1043_2000);
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f100, BusAccessType::DataLoad).unwrap(), 0x1043_3000);
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f104, BusAccessType::DataLoad).unwrap(), 0x1043_4000);
    assert_eq!(
        bus.read(RvSize::Word, 0xcafe_f108, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::HalfWord, 0xcafe_f0f6, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0e0, BusAccessType::DataLoad).unwrap(), 0x4de0d4f5);
    assert_eq!(bus.read(RvSize::HalfWord, 0xcafe_f0e0, BusAccessType::DataLoad).unwrap(), 0x4de0d4f5);
    assert_eq!(
        bus.log.take(),
        "reg_action0 read Word; reg_action0 read HalfWord; "
    );

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0e4, BusAccessType::DataLoad).unwrap(), 0xa813_c333);

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f0e8, BusAccessType::DataLoad).unwrap(), 0xba5e_ba11);
    assert_eq!(bus.log.take(), "reg_action2 read Word; ");

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f114, BusAccessType::DataLoad).unwrap(), 0xf00d_0000);
    assert_eq!(bus.log.take(), "reg_array_action0[0] read Word; ");

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f118, BusAccessType::DataLoad).unwrap(), 0xf00d_0001);
    assert_eq!(bus.log.take(), "reg_array_action0[1] read Word; ");

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f11c, BusAccessType::DataLoad).unwrap(), 0x2001_1000);
    assert_eq!(bus.log.take(), "");

    assert_eq!(bus.read(RvSize::Word, 0xcafe_f120, BusAccessType::DataLoad).unwrap(), 0x2001_2000);
    assert_eq!(bus.log.take(), "");

    assert_eq!(
        bus.read(RvSize::Word, 0xcafe_f130, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f134, BusAccessType::DataLoad).unwrap(), 0xd00d_0000);
    assert_eq!(bus.log.take(), "reg_array_action2[0] read Word; ");
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f138, BusAccessType::DataLoad).unwrap(), 0xd00d_0001);
    assert_eq!(bus.log.take(), "reg_array_action2[1] read Word; ");
    assert_eq!(bus.read(RvSize::Word, 0xcafe_f144, BusAccessType::DataLoad).unwrap(), 0xd00d_0004);
    assert_eq!(bus.log.take(), "reg_array_action2[4] read Word; ");
    assert_eq!(
        bus.read(RvSize::Word, 0xcafe_f148, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
    assert_eq!(
        bus.read(RvSize::Byte, 0xcafe_f131, BusAccessType::DataLoad).unwrap_err(),
        BusError::LoadAccessFault
    );
}
...
fn test_write_dispatch() {
    ...
    assert_eq!(bus.rom.read(RvSize::Word, 0x3430, BusAccessType::DataLoad).unwrap(), 0x3828_abcd);

    bus.write(RvSize::HalfWord, 0x1000_0002, 0x892a).unwrap();
    bus.write(RvSize::HalfWord, 0x1000_0000, 0xb38a).unwrap();
    assert_eq!(bus.sram.read(RvSize::Word, 0x0, BusAccessType::DataLoad).unwrap(), 0x892a_b38a);
    ...
    assert_eq!(bus.dram.read(RvSize::Word, 0xfffc, BusAccessType::DataLoad).unwrap(), 0x2a29_3072);

    bus.write(RvSize::Word, 0xaa00_0020, 0x8e27_ab42).unwrap();
    assert_eq!(bus.uart0.read(RvSize::Word, 0x20, BusAccessType::DataLoad).unwrap(), 0x8e27_ab42);

    bus.write(RvSize::Word, 0xaa01_0074, 0x38b8_e201).unwrap();
    assert_eq!(bus.uart1.read(RvSize::Word, 0x74, BusAccessType::DataLoad).unwrap(), 0x38b8_e201);

    bus.write(RvSize::Word, 0xaa02_001c, 0xc1a4_823f).unwrap();
    assert_eq!(bus.i2c0.read(RvSize::Word, 0x1c, BusAccessType::DataLoad).unwrap(), 0xc1a4_823f);

    bus.write(RvSize::Word, 0xaa02_0068, 0x0e8a_440b).unwrap();
    assert_eq!(bus.i2c0.read(RvSize::Word, 0x68, BusAccessType::DataLoad).unwrap(), 0x0e8a_440b);

    bus.write(RvSize::Word, 0xaa02_0400, 0x0e8a_440b).unwrap();
    assert_eq!(bus.i2c1.read(RvSize::Word, 0x00, BusAccessType::DataLoad).unwrap(), 0x0e8a_440b);

    bus.write(RvSize::Word, 0xaa02_0854, 0x70fa_81c9).unwrap();
    assert_eq!(bus.i2c2.read(RvSize::Word, 0x54, BusAccessType::DataLoad).unwrap(), 0x70fa_81c9);

    bus.write(RvSize::Word, 0xbb42_d87c, 0x48ba_38c1).unwrap();
    assert_eq!(bus.spi0.read(RvSize::Word, 0xd87c, BusAccessType::DataLoad).unwrap(), 0x48ba_38c1);
