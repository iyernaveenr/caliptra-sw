...
impl Bus for DynamicBus {
    fn read(&mut self, size: RvSize, addr: RvAddr, _access_type: BusAccessType) -> Result<RvData, BusError> {
        ...
        match dev {
            Some(dev) => dev.bus.read(size, addr - dev.mmap_range.start(), _access_type),
...
mod test {
    ...
    fn test_dynamic_bus_read() {
        ...
        assert_eq!(bus.read(RvSize::Byte, 1, BusAccessType::DataLoad).ok(), Some(1));
        assert_eq!(bus.read(RvSize::Byte, 2, BusAccessType::DataLoad).ok(), Some(2));
        assert_eq!(
    ...
    fn test_dynamic_bus_write() {
        ...
        assert_eq!(bus.read(RvSize::Byte, 1, BusAccessType::DataLoad).ok(), Some(3));
        assert_eq!(bus.write(RvSize::Byte, 2, 4).ok(), Some(()));
        assert_eq!(bus.read(RvSize::Byte, 2, BusAccessType::DataLoad).ok(), Some(4));
        assert_eq!(
