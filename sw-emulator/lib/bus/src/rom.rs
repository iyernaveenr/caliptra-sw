...
mod tests {
    ...
    fn test_read() {
        ...
        assert_eq!(rom.read(RvSize::Byte, 0, BusAccessType::DataLoad).ok(), Some(1));
        assert_eq!(rom.read(RvSize::HalfWord, 0, BusAccessType::DataLoad).ok(), Some(1 | (2 << 8)));
        assert_eq!(
    ...
    fn test_read_error() {
        let mut rom = Rom::new(vec![1, 2, 3, 4]);
        assert_eq!(
            rom.read(RvSize::Byte, rom.len() as RvAddr, BusAccessType::DataLoad).err(),
