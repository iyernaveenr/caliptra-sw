...
impl<TBus: Bus> Bus for MracBus<TBus> {
    fn read(&mut self, size: RvSize, addr: RvAddr, access_type: BusAccessType) -> Result<RvData, BusError> {
        if !self.enabled {
        ...
        match access_type {
            BusAccessType::InstrFetch => {
                ...
                if self.is_cacheable(addr) {
                    ...
                    let hi = self.inner.read(RvSize::Word, aligned_addr + 4, access_type)?;
...
mod test {
    ...
    fn test_disabled_passthrough() {
        ...
        assert!(bus.read(RvSize::Byte, 0x3003_0001, BusAccessType::DataLoad).is_ok());
    ...
    fn test_sideeffect_word_aligned_ok() {
        ...
        assert!(bus.read(RvSize::Word, 0x3003_0000, BusAccessType::DataLoad).is_ok());
    ...
    fn test_sideeffect_halfword_aligned_ok() {
        ...
        let mrac_val = 1 << 7; // sideeffect for region 3
        let mut bus = make_mrac_bus(mrac_val, true);
        assert!(bus.read(RvSize::HalfWord, 0x3003_0000, BusAccessType::DataLoad).is_ok());
    }
    ...
    fn test_cacheable_read_widens() {
        ...
        assert!(bus.read(RvSize::Byte, 0x5000_0001, BusAccessType::DataLoad).is_ok());
        assert!(bus.read(RvSize::HalfWord, 0x5000_0002, BusAccessType::DataLoad).is_ok());
        assert!(bus.read(RvSize::Word, 0x5000_0004, BusAccessType::DataLoad).is_ok());
    }
    ...
    fn test_no_flags_passthrough() {
        ...
        assert!(bus.read(RvSize::Byte, 0x2000_0001, BusAccessType::DataLoad).is_ok());
