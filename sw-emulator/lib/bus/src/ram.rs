...
mod tests {
    ...
    fn test_read() {
        ...
        assert_eq!(ram.read(RvSize::Byte, 0, BusAccessType::DataLoad).ok(), Some(1));
        assert_eq!(ram.read(RvSize::HalfWord, 0, BusAccessType::DataLoad).ok(), Some(1 | (2 << 8)));
        assert_eq!(
    ...
    fn test_read_error() {
        let mut ram = Ram::new(vec![1, 2, 3, 4]);
        assert_eq!(
            ram.read(RvSize::Byte, ram.len() as RvAddr, BusAccessType::DataLoad).err(),
