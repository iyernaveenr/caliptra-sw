...
mod tests {
    ...
    fn test_mldsa_name() {
        ...
        let name0 = ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_NAME0, BusAccessType::DataLoad).unwrap();
        let name0 = String::from_utf8_lossy(&name0.to_le_bytes()).to_string();
        assert_eq!(name0, "LMSD");

        let name1 = ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_NAME1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_mldsa_version() {
        ...
        let version0 = ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_VERSION0, BusAccessType::DataLoad).unwrap();
        let version0 = String::from_utf8_lossy(&version0.to_le_bytes()).to_string();
        assert_eq!(version0, ".100");

        let version1 = ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_VERSION1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_mldsa_control() {
        ...
        let mut ml_dsa87 = Abr::new(&clock, key_vault, sha512);
        assert_eq!(
            ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_CONTROL, BusAccessType::DataLoad).unwrap(),
            0
        );
    }
    ...
    fn test_mldsa_status() {
        ...
        let mut ml_dsa87 = Abr::new(&clock, key_vault, sha512);
        assert_eq!(ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_STATUS, BusAccessType::DataLoad).unwrap(), 1);
    }
    ...
    fn test_mldsa_gen_key() {
        ...
        loop {
            let status = InMemoryRegister::<u32, MlDsaStatus::Register>::new(
                ml_dsa87.read(RvSize::Word, OFFSET_MLDSA_STATUS, BusAccessType::DataLoad).unwrap(),
            );
        ...
        for key_id in 0..KeyVault::KEY_COUNT {
            ...
            loop {
                let seed_read_status = InMemoryRegister::<u32, KvStatus::Register>::new(
                    ml_dsa87
                        .read(RvSize::Word, OFFSET_MLDSA_KV_RD_SEED_STATUS, BusAccessType::DataLoad)
                        .unwrap(),
                );
    ...
    fn test_mlkem_keygen() {
        ...
        loop {
            let status = InMemoryRegister::<u32, MlKemStatus::Register>::new(
                abr.read(RvSize::Word, OFFSET_MLKEM_STATUS, BusAccessType::DataLoad).unwrap(),
            );
        ...
        let encaps_key_word = abr.read(RvSize::Word, OFFSET_MLKEM_ENCAPS_KEY, BusAccessType::DataLoad).unwrap();
        let decaps_key_word = abr.read(RvSize::Word, OFFSET_MLKEM_DECAPS_KEY, BusAccessType::DataLoad).unwrap();
    ...
    fn test_mlkem_encaps_decaps() {
        ...
        for (i, c) in ciphertext_readback.iter_mut().enumerate() {
            *c = abr
                .read(RvSize::Word, OFFSET_MLKEM_CIPHERTEXT + (i * 4) as RvAddr, BusAccessType::DataLoad)
                .unwrap();
        }
