...
mod tests {
    ...
    fn test_name() {
        ...
        let name0 = hmac.read(RvSize::Word, OFFSET_NAME0, BusAccessType::DataLoad).unwrap();
        let name0 = String::from_utf8_lossy(&name0.to_le_bytes()).to_string();
        assert_eq!(name0, "hmac");

        let name1 = hmac.read(RvSize::Word, OFFSET_NAME1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_version() {
        ...
        let version0 = hmac.read(RvSize::Word, OFFSET_VERSION0, BusAccessType::DataLoad).unwrap();
        let version0 = String::from_utf8_lossy(&version0.to_le_bytes()).to_string();
        assert_eq!(version0, "1.00");

        let version1 = hmac.read(RvSize::Word, OFFSET_VERSION1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_control() {
        let mut hmac = HmacSha::new(&Clock::new(), KeyVault::new());
        assert_eq!(hmac.read(RvSize::Word, OFFSET_CONTROL, BusAccessType::DataLoad).unwrap(), 0);
    }

    #[test]
    fn test_status() {
        let mut hmac = HmacSha::new(&Clock::new(), KeyVault::new());
        assert_eq!(hmac.read(RvSize::Word, OFFSET_STATUS, BusAccessType::DataLoad).unwrap(), 1);
    }
    ...
    fn test_tag() {
        ...
        for addr in (OFFSET_TAG..(OFFSET_TAG + HMAC_TAG_SIZE_384 as u32)).step_by(4) {
            assert_eq!(hmac.read(RvSize::Word, addr, BusAccessType::DataLoad).ok(), Some(0));
            assert_eq!(
        ...
        if !key_via_kv {
            ...
            loop {
                let key_read_status = InMemoryRegister::<u32, KeyReadStatus::Register>::new(
                    hmac.read(RvSize::Word, OFFSET_KEY_STATUS, BusAccessType::DataLoad).unwrap(),
                );
            ...
            if !block_via_kv {
                ...
                loop {
                    let block_read_status = InMemoryRegister::<u32, KeyReadStatus::Register>::new(
                        hmac.read(RvSize::Word, OFFSET_BLOCK_STATUS, BusAccessType::DataLoad).unwrap(),
                    );
            ...
            loop {
                if !tag_to_kv {
                    let status = InMemoryRegister::<u32, Status::Register>::new(
                        hmac.read(RvSize::Word, OFFSET_STATUS, BusAccessType::DataLoad).unwrap(),
                    );
                    ...
                    let tag_write_status = InMemoryRegister::<u32, TagWriteStatus::Register>::new(
                        hmac.read(RvSize::Word, OFFSET_TAG_STATUS, BusAccessType::DataLoad).unwrap(),
                    );
