...
impl Bus for Iccm {
    ...
    fn read(&mut self, size: RvSize, addr: RvAddr, _access_type: BusAccessType) -> Result<RvData, BusError> {
        self.iccm.ram.borrow_mut().read(size, addr, BusAccessType::DataLoad)
    }
...
mod tests {
    ...
    fn test_unlocked_write() {
        ...
        for word_offset in (0u32..ICCM_SIZE_BYTES as u32).step_by(4) {
            assert_eq!(iccm.read(RvSize::Word, word_offset, BusAccessType::DataLoad).unwrap(), 0);
            assert_eq!(
                iccm.write(RvSize::Word, word_offset, u32::MAX).ok(),
                Some(())
            );
            assert_eq!(iccm.read(RvSize::Word, word_offset, BusAccessType::DataLoad).ok(), Some(u32::MAX));
        }
