...
mod tests {
    ...
    fn test_name_read() {
        ...
        let name0 = sha256.read(RvSize::Word, OFFSET_NAME0, BusAccessType::DataLoad).unwrap();
        let mut name0 = String::from_utf8_lossy(&name0.to_le_bytes()).to_string();
        name0.pop();
        assert_eq!(name0, "256");

        let name1 = sha256.read(RvSize::Word, OFFSET_NAME1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_version_read() {
        ...
        let version0 = sha256.read(RvSize::Word, OFFSET_VERSION0, BusAccessType::DataLoad).unwrap();
        let version0 = String::from_utf8_lossy(&version0.to_le_bytes()).to_string();
        assert_eq!(version0, "1.00");

        let version1 = sha256.read(RvSize::Word, OFFSET_VERSION1, BusAccessType::DataLoad).unwrap();
    ...
    fn test_control_read() {
        let mut sha256 = HashSha256::new(&Clock::new());
        assert_eq!(sha256.read(RvSize::Word, OFFSET_CONTROL, BusAccessType::DataLoad).unwrap(), 0);
    }

    #[test]
    fn test_status_read() {
        let mut sha256 = HashSha256::new(&Clock::new());
        assert_eq!(sha256.read(RvSize::Word, OFFSET_STATUS, BusAccessType::DataLoad).unwrap(), 1);
    }
    ...
    fn test_block_read_write() {
        ...
        for addr in (OFFSET_BLOCK..(OFFSET_BLOCK + SHA256_BLOCK_SIZE as u32)).step_by(4) {
            ...
            assert_eq!(sha256.read(RvSize::Word, addr, BusAccessType::DataLoad).ok(), Some(u32::MAX));
        }
    }
    ...
    fn test_hash_read_write() {
        ...
        for addr in (OFFSET_HASH..(OFFSET_HASH + SHA256_HASH_SIZE as u32)).step_by(4) {
            assert_eq!(sha256.read(RvSize::Word, addr, BusAccessType::DataLoad).ok(), Some(0));
            assert_eq!(
        ...
        for idx in 0..totalblocks {
            ...
            loop {
                let status = InMemoryRegister::<u32, Status::Register>::new(
                    sha256.read(RvSize::Word, OFFSET_STATUS, BusAccessType::DataLoad).unwrap(),
                );
