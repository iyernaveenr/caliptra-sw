...
mod tests {
    ...
    fn test_get_image() {
        ...
        let a = rri.read(RvSize::Word, INDIRECT_FIFO_CTRL0, BusAccessType::DataLoad).unwrap();
        let b = rri.read(RvSize::Word, INDIRECT_FIFO_CTRL1, BusAccessType::DataLoad).unwrap();
        let image_size = (a & 0xffff_0000) | (b & 0xffff);
        assert_eq!(image_len, image_size as usize * 4);

        let mut read_image = Vec::new();
        while rri.read(RvSize::Word, INDIRECT_FIFO_STATUS, BusAccessType::DataLoad).unwrap() & 1 == 0 {
            let dword_read = rri.read(RvSize::Word, INDIRECT_FIFO_DATA, BusAccessType::DataLoad).unwrap();
