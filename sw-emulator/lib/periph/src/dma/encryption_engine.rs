...
macro_rules! assert_ctrl_eq {
    ($encryption_engine:ident, $target:expr) => {
        assert_eq!(
            $encryption_engine.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap(),
            $target.into()
        );
    };
}
...
mod tests {
    ...
    fn execute_command(encryption_engine: &mut EncryptionEngine, command: u32) -> Result<(), ()> {
        let value = encryption_engine.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap();
    ...
    fn wait_done(encryption_engine: &mut EncryptionEngine) -> Result<(), ()> {
        ...
        let mut value = encryption_engine.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap();
        while value & mask == Control::RDY::READY.value {
            value = encryption_engine.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap();
        }
    ...
    fn clear(encryption_engine: &mut EncryptionEngine) {
        let mut v = encryption_engine.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap();
    ...
    fn test_encryption_engine_initialization_fatal_error() {
        ...
        let ready_value = ee.read(RvSize::Word, CTRL, BusAccessType::DataLoad).unwrap() & Control::RDY::READY.value;
